...
/**
 * @Description 支持常见的圆形、圆角矩形（支持边框、支持按下态）
 * @Author 一花一世界
 */
public class RoundImageView extends ImageView {
    ...
    public RoundImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mRoundRadius = typedArray.getDimensionPixelSize(R.styleable.RoundImageView_roundRadius, dip2px(DEFAULT_ROUND_RADIUS));
        mBorderWidth = typedArray.getDimensionPixelSize(R.styleable.RoundImageView_borderWidth, dip2px(DEFAULT_BORDER_WIDTH));
    ...
    public void setRoundRadius(int roundRadius) {
        if (this.mRoundRadius != dip2px(roundRadius)) {
            this.mRoundRadius = dip2px(roundRadius);
            invalidate();
        }
    }
    ...
    public void setBorderWidth(int borderWidth) {
        if (this.mBorderWidth != dip2px(borderWidth)) {
            this.mBorderWidth = dip2px(borderWidth);
            mBorderPaint.setStrokeWidth(mBorderWidth);
            invalidate();
        }
    }
